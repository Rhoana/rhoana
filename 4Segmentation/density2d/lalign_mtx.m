...
function [score alignment alignment_points pointorph1_points pointorph2_points] = lalign_mtx_allnn_smooth_poster(lines1, lines2, xoffset, yoffset)
...
for i = 1:sizem
    if orph1(i)
        ...
        if i > 1 && (all1(i).lineix == all1(i-1).lineix)
            ...
            matchj(maxmj:maxmj+length(newmatch)) = newmatch;
...
for j = 1:sizen
    if orph2(j)
        ...
        if j > 1 && (all2(j).lineix == all2(j-1).lineix)
            ...
            matchi(maxmi:maxmi+length(newmatch)) = newmatch;
