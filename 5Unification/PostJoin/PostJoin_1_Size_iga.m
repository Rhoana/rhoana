dataset_dir = 'C:\dev\datasets\conn\ignacio\';
subvol_name = 'area2';
...
result_name = 'res_from_Nov27_scf07_seg5_PF';
%result_name = 'res_from_Dec3_scf08_seg51_PF';
%result_name = 'res_from_Nov09_PF';
...
%result_name = 'res_from_SmDi_J1_scf08_PF';
...
%else
    ...
    prob_files = [ dir(fullfile(prob_folder, '*.tif')) ];
    prob_files = sort({prob_files.name});
    
    for zi = 1:length(prob_files)
        imProb = imread(fullfile(prob_folder, prob_files{zi}));
        imProb = 1-norm01(imProb);
        imProb = norm01(log(imProb+0.1));
        prob_vol(:,:,zi) = imProb;
    end
    clear load_img;
...
for minsegsize = [0 25 100 500] %1000 2500 5000 10000 15000 20000 25000];
    %segoi = 1;
