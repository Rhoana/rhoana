...
while repeat_attempt_i < job_repeat_attempts and not check_file(output_path):
    ...
    try:
        ...
        max_prob = 2 ** 31
        if len(prob_image.shape) == 3:
            thresh_image = np.uint32((1 - prob_image[:,:,0]) * max_prob)
        else:
        ...
        if blankout_zero_regions:
            ...
            if len(blankable) > 0:
                blank_mask = np.zeros((prob_image.shape[0], prob_image.shape[1]), dtype=np.bool)
        ...
        probs_out = out_hdf5.create_dataset('probabilities',
                                                ...
                                                chunks = (64, 64, 1) if len(prob_image.shape) == 3 else (64, 64),
                                                compression='gzip')
