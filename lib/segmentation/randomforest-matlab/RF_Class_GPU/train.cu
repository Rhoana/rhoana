...
__device__ void sortbagbyx(
	const float *x, int n, int mdim, int featurei, int *bagstart, int ndstart, int ndend, int *tempbagstart)
{
	...
	while (i < length)
	{
		...
		for (start1 = ndstart; start1 <= ndend; start1 += i*2)
		{
			...
			{
				if (x[xstart + inbag[p1]] < x[xstart + inbag[p2]])
				{
	...
	if (idx < nbags)
	{
		...
		for (i = 0; i < nclass; ++i){
			for (j = 0; j < nsamples[i]; ++j) {
				bagstart[k] = classindex[bagstart[j + i * maxnsamples] + cioffset];
				++k;
			}
...
__device__ void findBestSplit(
	const float *x, int n, int mdim, int nclass, const int *classes, int *bagstart,
	...
	int *wlstart, int *wrstart, int *dimtempstart, int *tempbagstart)
{
	...
	*msplit = -1;
	...
	for (trynum = 0; trynum < mtry && trynum < mdim; ++trynum)
	{
		...
		sortbagbyx(x, n, mdim, featurei, bagstart, ndstart, ndend, tempbagstart);
		...
		for (splitpoint = ndstart; splitpoint <= ndend-1; ++splitpoint)
		{
			...
			thisx = x[splitxi + n * featurei];
			nextx = x[bagstart[splitpoint+1] + n * featurei];
			if (thisx != nextx)
			{
	...
	else if (*msplit != featurei)
	{
		...
		sortbagbyx(x, n, mdim, *msplit, bagstart, ndstart, ndend, tempbagstart);
	}
...
__global__ void trainKernel(
	...
	int *dimtemp, int *bagspace, int *tempbag)
{
// Optional arguments for debug (place after xbestsplit): int *nbestsplit, float *bestgini,
	...
	if (idx < ntree) {
		...
		for (i = 0; i < nclass; ++i){
			for (j = 0; j < nsamples[i]; ++j) {
				bagstart[k] = classindex[bagstart[j + i * maxnsamples] + cioffset];
				//TODO: Move memory into local block?
				//or do this on CPU?
				++k;
			}
			cioffset += samplefrom[i];
			//classpop[i + 0 * nclass + idx * nclass * maxTreeSize] = nsamples[i];
		...
		for (k = 0; k < maxTreeSize-2; ++k) {
			...
			findBestSplit(x, n, mdim, nclass, classes, bagstart, totsamples, k, ndstart, ndend, &ndendl,
			...
			for (i = ndstart; i <= ndendl; ++i) {
				nodeclass = classes[bagstart[i]]-1;
