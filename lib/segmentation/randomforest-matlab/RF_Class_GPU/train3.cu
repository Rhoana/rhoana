...
__device__ void sampledata(const int nclass, const int* nsamples, const int* samplefrom,
	int* bagstart, curandState_t *randstate)
{
	...
	for (iclass=0; iclass < nclass; ++iclass) {
		nsamp += nsamples[iclass];
		while (isamp < nsamp) {
			bagstart[isamp] = curand(randstate) % samplefrom[iclass];
			++isamp;
		}
	}
}
...
__global__ void randomsample(const int nclass, const int* nsamples, const int* samplefrom,
	const int totsamples, int* bagspace, int nbags,
	const unsigned long long seed, const unsigned long long sequencestart)
{
	...
	if (idx < nbags)
	{
		...
		sampledata(nclass, nsamples, samplefrom, bagspace + idx * totsamples, &state);
	}
	...
	int* tempbag, const int* classindex, float* baggedx)
{
	...
	if (idx < nbags)
	{
		...
		int i,k,cioffset,nsamp,xindex,dimindex;

		int *bagstart = bagspace + idx * totsamples;
		int *tempbagstart = tempbag + idx * totsamples;
		float *baggedxstart = baggedx + idx * mdim * totsamples;
		sampledata(nclass, nsamples, samplefrom, bagstart, &state);


		//Index into x (instead of into class)
		k = 0;
		cioffset = 0;
		nsamp = 0;
		for (i = 0; i < nclass; ++i){
			...
			while (k < nsamp)
			{
				xindex = classindex[bagstart[k] + cioffset];
				//Move memory into local block
				for (dimindex = 0; dimindex < mdim; ++dimindex){
					baggedxstart[k + totsamples * dimindex] = x[xindex + n * dimindex];
				}
				//Change index
				bagstart[k] = k;
				//bagstart[k] = xindex;
		...
		//bagstart[(nclass * totsamples)-1] = featurei;
		//bagstart[(nclass * totsamples)-2] = featureTest;
	}
...
// Optional arguments for debug (place after xbestsplit): int *nbestsplit, float *bestgini,
	...
	if (idx < ntree) {
		...
		int *bagstart = bagspace + idx * nclass * totsamples;
		int *tempbagstart = tempbag + idx * nclass * totsamples;
		float *baggedxstart = baggedx + idx * mdim * totsamples;
		...
		for (i = 0; i < nclass; ++i){
			...
			while (k < nsamp)
			{
				...
				//Change index
				baggedclassstart[k] = classes[xindex];
				bagstart[k] = k;
				//bagstart[k] = xindex;
				++k;
			}
