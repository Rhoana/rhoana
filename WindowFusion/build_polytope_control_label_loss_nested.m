function [lhs_polytope, ...
         A_polytope, ...
         rhs_polytope] = build_polytope_control_label_loss_nested(...
   vars_2D_cliques, ....
   vars_3D_links, ...
   n_variables, ...
   link_As, ...
...
% # amvr FIX_ME
% This still operates in single axis mode!
...
A_cols   = cell(2*n_2D_cliques, 1);
A_values = cell(2*n_2D_cliques, 1);
rhs_values    = zeros(2*n_2D_cliques, 1);

ineq_ix = 0;
for clique_2D_ix = 1:n_2D_cliques
   this_cube_ix = clique_ix_to_cube_ix(clique_2D_ix);
  ...
  if  this_cube_ix >1
     ...
     % Segment:
     this_clique_vars         = reshape(vars_2D_cliques(clique_2D_ix), [],1);
     ineq_cols_1              = this_clique_vars;
     vals_1                   = ones(numel(this_clique_vars), 1);

     % Links
     this_clique_links_vars  = reshape(vars_3D_links(which_links_bckwd), [],1);
     ...
     % this_clique_links_to_vars    =  vars_2D_cliques(this_clique_links_to_cliques);
     this_clique_links_to_areas   = clique_2D_iface_sizes(this_clique_links_to_cliques,RGHT);
     this_clique_area             = clique_2D_iface_sizes(clique_2D_ix,LEFT);  
...
%      vals_1 = this_clique_area * vals_1 - loose_margin*this_clique_area;
  ...
  if this_cube_ix < n_cubes_to_process
     %% ---------------  
     %%   Forward links:
     %% ---------------
     % Segment
     ...
     % Links
     this_clique_vars         = reshape(vars_3D_links(which_links_fwd), [],1);
     ...
     % this_clique_links_to_vars    =  vars_2D_cliques(this_clique_links_to_cliques);
     this_clique_links_to_areas   = clique_2D_iface_sizes(this_clique_links_to_cliques, LEFT);
     this_clique_area             = clique_2D_iface_sizes(clique_2D_ix, RGHT);  
